...
function registrarEndpointGuardarLaboratorio(app, supabase) {
    app.post('/savePracticasLaboratorio', async (req, res) => {
        try {
            ...
            if (Array.isArray(archivosPDF)) {
                for (const archivo of archivosPDF) {
                    try {
                        ...
                        const nombreLimpio = (archivo.nombre || 'informe.pdf')
                        ...
                        if (!uploadError) {
                            ...
                            console.warn('Error subiendo PDF a Storage:', uploadError.message);
                        }
                    } catch (e) {
            ...
            if (errorHistorico) {
                console.error('Error insertando en practicas_historicas:', errorHistorico.message);
